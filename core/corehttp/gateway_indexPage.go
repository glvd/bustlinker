...
func breadcrumbs(urlPath string, dnslinkOrigin bool) []breadcrumb {
	...
	// Drop the /blns/<fqdn> prefix from breadcrumb Paths when directory
...
// (when hostname from gwURL is matching /blns/<fqdn> in path)
func hasDNSLinkOrigin(gwURL string, path string) bool {
	if gwURL != "" {
		fqdn := stripPort(strings.TrimPrefix(gwURL, "//"))
		return strings.HasPrefix(path, "/blns/"+fqdn)
	}
	return false
}
...
func init() {
	...
	iconFromExt := func(name string) string {
		...
		return "link-" + ext[1:] // slice of the first dot
	}
